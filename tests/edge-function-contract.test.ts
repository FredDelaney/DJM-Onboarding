...
test("every deployed Edge Function has source and explicit JWT configuration", () => {
  ...
  assert.deepEqual(functionNames, [
    ...
    "refresh-player-stats-ai-worker",
...
test("football providers are capability-gated and never expose Wyscout secrets to the client", () => {
  const importFunction = readFileSync(
    new URL(
      "../supabase/functions/import-player-stats/index.ts",
      import.meta.url,
    ),
    "utf8",
  );
  const transfermarktFunction = readFileSync(
    new URL(
      "../supabase/functions/djm-transfermarkt-enrich/index.ts",
      import.meta.url,
    ),
    "utf8",
  );
  const providers = readFileSync(
    new URL(
      "../supabase/functions/_shared/football-data/providers.ts",
      import.meta.url,
    ),
    "utf8",
  );
  const wyscout = readFileSync(
    new URL(
      "../supabase/functions/_shared/football-data/wyscout.ts",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  assert.match(
    transfermarktFunction,
    /Legacy parser retained for a possible future licensed integration/,
  );
