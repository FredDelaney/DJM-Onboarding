...
export function AdminShell({children}:{children:React.ReactNode}){const router=useRouter();return <div className="admin-shell"><div className="admin-head"><div className="container admin-top"><Link href="/admin"><Brand/></Link><div className="row"><span className="pill pill-dark">DJM ADMIN</span><button className="icon-btn" onClick={async()=>{await supabase.auth.signOut();router.replace('/sign-in')}}><LogOut size={17}/></button></div></div></div>{children}</div>}
