...
export default function QuickCapture() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      platformRpc<any[]>('djm_network_club_contacts', { p_search: null, p_limit: 300 }),
  ...
  const submitCapture = async (event: FormEvent) => {
    ...
    try {
      ...
      if (file) {
        ...
        payload = await platformInvoke('djm-network-capture', form);
      } else {
        payload = await platformInvoke('djm-network-capture', {
          text: text.trim(),
          channel,
      ...
      setMessage(
        [
          ...
          linked ? `Linked to ${linked}.` : 'ReDream will keep it in context and review only if needed.',
          created ? `${created}.` : '',
  ...
  const submitRecruitment = async (event: FormEvent) => {
    ...
    try {
      const quick: any = await platformRpc('djm_recruitment_quick_add', {
      ...
      if (quick?.prospect_id) {
        try {
          enrichment = await platformInvoke('djm-transfermarkt-enrich', {
        ...
        <div className="djm-os-search-overlay" onMouseDown={close}>
          <div className="djm-os-capture-modal" onMouseDown={(e) => e.stopPropagation()}>
            <div className="djm-os-capture-head">
              <div>
                <strong>Add to ReDream</strong>
                <p>Give ReDream the raw information. The system should do the admin.</p>
              </div>
            ...
            <div className="djm-os-capture-body">
                ...
                <form className="djm-os-form" onSubmit={submitCapture}>
                  ...
                  <details style={{ border: '1px solid var(--djm-line)', borderRadius: 12, padding: 12 }}>
                    <summary style={{ cursor: 'pointer', fontSize: 11, fontWeight: 800, color: 'var(--djm-navy)' }}>
                      Attach manually only if ReDream cannot infer it
                    </summary>
                  ...
                  <button className="djm-os-primary-button" type="submit" disabled={busy || (!text.trim() && !file)}>
                    <FileUp size={15} /> {busy ? 'Adding…' : 'Add to ReDream'}
                  </button>
                </form>
              ) : null}
              ...
              {mode === 'recruitment' ? (
                <form className="djm-os-form" onSubmit={submitRecruitment}>
                  ...
                  <div className="djm-os-form-grid">
                    ...
                    <label>
                      Optional note
                      <input value={playerNotes} onChange={(e) => setPlayerNotes(e.target.value)} placeholder="Why ReDream should approach him" />
                    </label>
                  </div>
                  <p style={{ margin: 0, color: 'var(--djm-muted)', fontSize: 10, lineHeight: 1.5 }}>
                    ReDream creates the Recruitment target immediately, then fills name, age, club, position, foot, contract, market value and agent data when available.
                  </p>
              ...
              {mode === 'need' ? (
                <form className="djm-os-form" onSubmit={submitNeed}>
                  ...
                  <p style={{ margin: 0, color: 'var(--djm-muted)', fontSize: 10, lineHeight: 1.5 }}>
                    ReDream extracts the position, foot, age and transfer type it can prove from your note. The original wording is always kept and player matching runs automatically.
                  </p>
