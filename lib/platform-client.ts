...
import { measureOperation } from '@/lib/operation-performance';

export async function platformRpc<T = any>(
  name: string,
...
export async function platformInvoke<T = any>(
  ...
  return measureOperation('edge', functionName, async () => {
    ...
    if (error) {
      let message = error.message || `Workspace function failed: ${functionName}`;
