...
function aiOutputTokenCap(model: string) {
...
async function transcribe(
  ...
  if (capture.capture_type !== "audio") {
    ...
    if (!typed) throw new Error("Typed ReDream AI capture has no text");
    return typed;
  }
  ...
  const file = new File([await data.arrayBuffer()], `ai-capture.${extension}`, {
    type: mime,
  });
...
async function interpret(
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      max_output_tokens: aiOutputTokenCap(model),
      instructions: [
        "You convert an internal football-agency debrief into safe ReDream actions.",
  ...
  if (!plan || !Array.isArray(plan.actions) || typeof plan.summary !== "string") {
    throw new Error("Interpreter returned an invalid ReDream AI plan");
  }
...
async function resolveEntity(
  ...
  const { data, error } = await admin.rpc("redream_ai_capture_resolve_entity", {
...
async function recordQuestion(
  ...
  const { data, error } = await admin.rpc("redream_ai_record_question", {
...
async function forceReviewAction(
  ...
  const { data, error } = await admin.rpc("redream_ai_apply_action", {
    p_capture_id: capture.capture_id,
...
async function getPlan(
  ...
  if (!transcript) {
    ...
    const { error: storeTranscriptError } = await admin.rpc(
      "redream_ai_worker_store_transcript",
      {
        p_capture_id: capture.capture_id,
  ...
  const aiRoute = selectAiRoute('ai_capture', { text: transcript });
  ...
  const modelCost = estimateAiCost(aiRoute, inputTokens, outputTokens);
  ...
  const { error: storePlanError } = await admin.rpc("redream_ai_worker_store_plan", {
    p_capture_id: capture.capture_id,
    p_transcript: transcript,
    p_plan: plan,
    p_usage: {
...
async function notifyAttention(admin: any, supabaseUrl: string, captureId: string) {
  const { data, error } = await admin.rpc("redream_ai_notify_attention", {
    p_capture_id: captureId,
  });
  if (error) {
    console.warn(JSON.stringify({
      operation: "redream_ai_notify_attention",
...
async function processOne(
  ...
  console.info(JSON.stringify({ operation: "redream_ai_claimed", capture_id: capture.capture_id, tenant_id: capture.tenant_id }));
  ...
  try {
    ...
    for (const item of orderedActions) {
        ...
        if (club.review) {
          ...
          await recordQuestion(
            ...
            club.candidates.length
              ? "ReDream found more than one plausible club or could not resolve it confidently."
              : "ReDream could not find this club in Network and will not invent a match.",
            candidates,
        ...
        if (!club.id && action.club_name && !club.review) {
          ...
          await recordQuestion(
            ...
            contact.candidates.length
              ? "ReDream will not guess between people."
              : "ReDream could not find this contact in Network.",
            candidates,
      ...
      if (isScoutObservation && !action.player_name) {
        ...
        await recordQuestion(
          ...
          "ReDream found similar Recruitment targets and will not create a duplicate without checking.",
        ...
        for (const field of ["salary_budget", "transfer_budget"] as const) {
          ...
          if (raw && !resolution.resolved && (action[field] == null || financialMagnitudeIsAmbiguous(raw))) {
            await recordQuestion(
              ...
              resolution.key,
              `What did “${raw}” mean?`,
              "ReDream heard a financial amount but will not guess its magnitude.",
      ...
      if (
        ...
        await recordQuestion(
          ...
          "ReDream will not guess a currency for financial information.",
    ...
    const { data: completed, error: completeError } = await admin.rpc(
      "redream_ai_worker_complete",
      {
        p_capture_id: capture.capture_id,
        p_transcript: transcript,
        p_summary: plan.summary || "ReDream captured your update",
        p_usage: usage,
      },
    );
    ...
    const message = error instanceof Error ? error.message : "ReDream AI processing failed";
    const retryable = isRetryable(error);
    const { data: failed } = await admin.rpc("redream_ai_worker_fail", {
      p_capture_id: capture.capture_id,
      p_error: message,
...
export async function handleAiProcess(request: Request) {
  ...
  const suppliedCron = request.headers.get("x-redream-cron") || request.headers.get("x-djm-cron") || "";
  ...
  if (mode === "cleanup") {
    ...
    for (const item of due || []) {
      ...
      if (!result.error) {
        const { error: markError } = await admin.rpc("redream_ai_mark_audio_deleted", {
    ...
    const { data: orphanDue, error: orphanError } = await admin.rpc(
      "redream_ai_orphan_audio_cleanup_due",
      { p_limit: 100 },
    );
    if (orphanError) {
  ...
  if (!openAiKey) {
    return json({ error: "OPENAI_API_KEY is not configured for ReDream AI" }, 503);
  }
