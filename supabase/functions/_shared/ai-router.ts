...
type RouteInput = {
  text?: string | null;
  forceTier?: AiTier | null;
};

const envNumber = (name: string, fallback: number) => {
  const value = Number((Deno.env.get(name.replace(/^DJM_/, 'REDREAM_')) ?? Deno.env.get(name)));
  return Number.isFinite(value) && value >= 0 ? value : fallback;
};

const ROUTES: Record<AiTier, AiRoute> = {
  fast: {
    tier: 'fast',
    model: (Deno.env.get('REDREAM_AI_FAST_MODEL') || Deno.env.get('DJM_AI_FAST_MODEL')) || 'gpt-5.6-luna',
  ...
  balanced: {
    tier: 'balanced',
    model: (Deno.env.get('REDREAM_AI_BALANCED_MODEL') || Deno.env.get('DJM_AI_BALANCED_MODEL')) || 'gpt-5.6-terra',
    reasoning_effort: 'none',
  ...
  deep: {
    tier: 'deep',
    model: (Deno.env.get('REDREAM_AI_DEEP_MODEL') || Deno.env.get('DJM_AI_DEEP_MODEL')) || 'gpt-5.6-sol',
...
function complexityScore(task: AiTask, text: string) {
  let score = 0;
