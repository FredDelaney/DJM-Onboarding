...
export async function handleAiCapture(req: Request) {
  ...
  try {
    ...
    if (!access?.enabled) {
      return json({ error: "ReDream AI is not enabled for this account" }, 403);
    }
    ...
    if (file instanceof File) {
      ...
      if (!supportedAudioMimes.has(baseMime)) {
        return json({ error: "This audio format is not supported by ReDream AI" }, 400);
      }
      ...
      if (bucketInfoError || !bucketInfo) {
        ...
        throw new Error("ReDream AI capture bucket must remain private");
      }
    ...
    if (captureId) {
      EdgeRuntime.waitUntil(
        fetch(`${url}/functions/v1/redream-ai-process`, {
          method: "POST",
          headers: {
          ...
          console.warn(JSON.stringify({
            operation: "redream_ai_kick_worker",
    ...
    console.error(JSON.stringify({
      operation: "redream_ai_capture",
      status: "failed",
      error: error instanceof Error ? error.message : "ReDream AI capture failed",
    }));
    return json(
