...
Deno.serve(async(req:Request)=>{
  ...
  if(configError||!cfg?.public_key||!cfg?.private_key) return new Response(JSON.stringify({error:"Push configuration missing"}),{status:500,headers:cors});
  ...
  for(const item of items||[]){
    ...
    for(const sub of subs){
      try{
        await webpush.sendNotification({endpoint:sub.endpoint,keys:{p256dh:sub.p256dh,auth:sub.auth_secret}},JSON.stringify({title:item.title,body:item.body,url:item.url,tag:`djm-${item.id}`}),{TTL:3600,urgency:"normal"});
        delivered++;
      }catch(e:any){
        const code=e?.statusCode||e?.status;
        errors.push(`${code||"error"}: ${e?.message||"push failed"}`);
        if(code===404||code===410) await db.from("push_subscriptions").update({enabled:false,updated_at:new Date().toISOString()}).eq("id",sub.id);
      }
    }
    if(delivered>0){
