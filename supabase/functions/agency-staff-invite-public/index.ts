...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (inviteError) {
      console.error(
        "agency-staff-invite-public preflight",
        inviteError.message,
      );
    ...
    if (existingId) {
      return reply(
        {
          error:
            "An account already exists for this email. Sign in to accept the agency invitation.",
          account_exists: true,
        },
        409,
      );
    ...
    return reply(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to complete agency invitation",
      },
      500,
    );
  }
