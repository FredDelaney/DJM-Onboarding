...
as $$
  select greatest(
    coalesce((select greatest(0,least(1,s.identity_confidence))*case when s.identity_verified_at is null then .75 when s.identity_verified_at>=now()-interval '2 years' then 1.0 when s.identity_verified_at>=now()-interval '5 years' then .85 else .65 end from djm_os.football_intelligence_subjects s where s.id=p_subject_id),0),
    coalesce((select max(greatest(0,least(1,e.confidence))) from djm_os.football_subject_identity_evidence e where e.subject_id=p_subject_id),0)
  );
$$;
...
begin
 ...
 v_source_diversity:=least(1::numeric,(case when v_comp is not null then .20 else 0 end)+(case when snap.id is not null then .24 else 0 end)+(case when v_identity_q>=.75 then .12 else 0 end)+(case when v_peers>=6 then .15 else 0 end)+(case when v_market is not null then .14 else 0 end)+(case when v_career is not null then .15 else 0 end));
 v_conf:=round(100*least(1,.27*least(1,v_observed_weight/100.0)+.13*v_country_q+.19*v_identity_q+.14*v_role_q+.07*v_peer_q+.04*v_market_q+.08*v_career_q+.08*v_source_diversity));
