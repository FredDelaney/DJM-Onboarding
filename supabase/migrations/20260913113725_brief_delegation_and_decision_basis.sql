create or replace function public.platform_server_agency_brief(p_tenant_id uuid, p_window_hours integer default 24, p_decision_limit integer default 5)
returns jsonb
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  v_judgement := public.platform_server_judgement_boundary(p_tenant_id,v_limit);
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'base_priority_score',(x.value->>'base_priority_score')::integer,
    'priority_score',(x.value->>'priority_score')::integer,
    'priority_band',x.value->>'priority_band',
    'decision_basis',x.value->'decision_basis',
  ...
  with events as (
    ...
    union all
    select e.created_at,'delegated',c.value from events e cross join lateral jsonb_array_elements(e.delegated_commands) c
    union all
    ...
    order by command_id,created_at desc,
      case movement_type when 'new' then 5 when 'escalated' then 4 when 'delegated' then 3 when 'deescalated' then 2 else 1 end desc
  )
  select coalesce(jsonb_agg(jsonb_build_object(
  ...
  with active_deals as (
    ...
    select d.id,d.title,d.stage,d.status,d.expected_commission,d.currency,
      coalesce(d.probability,d.manual_probability,d.model_probability,0) as probability,
      d.primary_blocker,d.next_action_text,d.next_action_at,d.last_meaningful_at
    from djm_os.deal_rooms d
    where d.tenant_id=p_tenant_id and d.status='active'
    order by
      case when d.next_action_at is null or d.next_action_at<now() then 0 else 1 end,
  ...
  select jsonb_build_object(
    ...
    ) order by currency) from active_deals),'[]'::jsonb),
    ...
    'deal_focus',coalesce((select jsonb_agg(jsonb_build_object(
      'deal_room_id',id,
      'title',title,
      'stage',stage,
      'expected_commission',expected_commission,
  ...
  return jsonb_build_object(
    ...
    'judgement_boundary',v_judgement,
    'movement',jsonb_build_object(
      'events',v_movements,
      'new_count',(select count(*) from jsonb_array_elements(v_movements) e where e.value->>'movement_type'='new'),
      'delegated_count',(select count(*) from jsonb_array_elements(v_movements) e where e.value->>'movement_type'='delegated'),
    ...
    'truth_contract',jsonb_build_object(
      'priorities','deterministic_rule_engine_plus_explainable_decision_value',
      'commercial_values','database_records_only',
      'ai_cost','metered_usage_ledger',
      'delegated_is_not_resolved',true,
