...
begin
  with pursuits as (
    ...
    select
      ...
      case
        when coalesce((a.access->'best_route'->>'route_score')::integer,0)<60 then 3
        when coalesce((a.access->'best_route'->>'route_score')::integer,0)>=75 then 2
        else 0
      end as policy_modifier,
      d.title,
      case
      ...
      case
        when coalesce((a.access->'best_route'->>'route_score')::integer,0)<60
      ...
      jsonb_build_object(
        'deal_room_id',d.id,'organisation_id',d.organisation_id,'stage',d.stage,'probability',d.probability,
        'expected_commission',d.expected_commission,'currency',d.currency,'primary_blocker',d.primary_blocker,
        'next_action_text',d.next_action_text,'next_action_at',d.next_action_at,'access',a.access
      ) as evidence
    from djm_os.deal_rooms d
    cross join lateral (select public.platform_server_access_routes(p_tenant_id,d.organisation_id,3) as access) a
