...
begin
  ...
  select exists (
    ...
    where pa.tenant_id=p_tenant_id and pa.status='active' and pf.feature_key='ai_assistant' and pf.enabled
  ) into v_ai_in_plan;
  ...
  select exists (select 1 from platform.ai_usage_events a where a.tenant_id=p_tenant_id and a.feature_key='ai_assistant' and a.status='success') into v_tell_djm_used;
  ...
  v_status := case
    when v_trial_success then 'full_value_seen'
    when v_owner_active and v_player_loaded and (v_market_workflow or (v_ai_in_plan and v_tell_djm_used)) then 'value_seen'
    when v_owner_active and v_player_loaded then 'activated'
    else 'not_activated'
  end;
  ...
  return jsonb_build_object(
    ...
    'customer_stage',v_stage,
    'status',v_status,
    'trial_success',v_trial_success,
    'core_milestones',jsonb_build_object(
      'brand_ready',v_brand_ready,
      'owner_active',v_owner_active,
      'player_data_loaded',v_player_loaded,
      'player_portal_live',v_player_portal,
      'tell_djm_used',case when v_ai_in_plan then v_tell_djm_used else null end,
      'market_workflow_used',v_market_workflow
    ),
    'expansion_signals',jsonb_build_object(
      'team_collaboration',v_team_collaboration,
      'custom_domain_live',v_custom_domain
    ),
    'success_definition',jsonb_build_object(
      'owner_active',true,
      'player_data_loaded',true,
      'player_portal_live',true,
      'market_workflow_used',true,
      'tell_djm_required',v_ai_in_plan
    ),
