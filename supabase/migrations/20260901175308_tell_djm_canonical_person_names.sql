-- Tell DJM canonical person names.
-- Keep rich picker labels for humans while storing clean canonical names in actions.
...
with rewritten as (
  select
    ...
    jsonb_agg(
      case
        when item->>'player_name' like '% · %' then
          jsonb_set(
            item,
            '{player_name}',
            to_jsonb(split_part(item->>'player_name',' · ',1)),
            true
          )
        else item
      end
  ...
  cross join lateral jsonb_array_elements(
    coalesce(c.extracted_json->'tell_djm_plan'->'actions','[]'::jsonb)
  ) with ordinality as x(item,ord)
...
where c.id=r.id
  and exists (
    ...
    where item->>'player_name' is not null
  );
