...
begin
  ...
  into v_active_deals,v_deal_items
  ...
  select count(*)::integer into v_authority from public.player_agreements a
  where a.player_id=p_player_id and a.status='active' and a.agreement_type in ('representation','mandate','placement_authorisation') and (a.end_date is null or a.end_date>=current_date);
  ...
  select count(*)::integer,count(*) filter(where t.due_at is not null and t.due_at<now())::integer into v_open_tasks,v_overdue_tasks
  from djm_os.tasks t where t.tenant_id=p_tenant_id and t.player_id=p_player_id and t.status='open';
  ...
  if v_p.primary_staff_user_id is null then v_gaps:=v_gaps||jsonb_build_array('primary_staff_not_assigned'); v_service_score:=v_service_score-12; end if;
  if nullif(trim(v_p.next_action),'') is null or v_p.next_action_due is null then v_gaps:=v_gaps||jsonb_build_array('player_next_action_not_controlled'); v_service_score:=v_service_score-22;
  elsif v_p.next_action_due<current_date then v_gaps:=v_gaps||jsonb_build_array('player_next_action_overdue'); v_service_score:=v_service_score-25;
  elsif v_p.next_action_due=current_date then v_gaps:=v_gaps||jsonb_build_array('player_next_action_due_today'); v_service_score:=v_service_score-8; end if;
  if v_overdue_requests>0 then v_gaps:=v_gaps||jsonb_build_array('overdue_player_request'); v_service_score:=v_service_score-least(25,v_overdue_requests*10); end if;
  if v_overdue_tasks>0 then v_gaps:=v_gaps||jsonb_build_array('overdue_player_task'); v_service_score:=v_service_score-least(15,v_overdue_tasks*5); end if;
  if v_market_trigger='free_agent' and v_active_deals=0 then v_gaps:=v_gaps||jsonb_build_array('free_agent_without_active_deal'); v_service_score:=v_service_score-18; end if;
  if v_market_trigger='contract_critical_window' and v_active_deals=0 and v_market_matches=0 and v_active_opps=0 then v_gaps:=v_gaps||jsonb_build_array('contract_critical_without_market_coverage'); v_service_score:=v_service_score-20; end if;
  if v_authority=0 then v_gaps:=v_gaps||jsonb_build_array('active_authority_record_not_found_in_platform'); v_service_score:=v_service_score-8; end if;
  if v_docs=0 then v_gaps:=v_gaps||jsonb_build_array('current_player_document_record_not_found'); v_service_score:=v_service_score-5; end if;
  ...
  return jsonb_build_object(
    ...
    'service_control',jsonb_build_object('score',v_service_score,'state',v_service_state,'gaps',v_gaps,'open_tasks',v_open_tasks,'overdue_tasks',v_overdue_tasks,'open_player_requests',v_open_requests,'overdue_player_requests',v_overdue_requests),
    ...
    'market_coverage',jsonb_build_object('state',v_coverage_state,'active_deals',v_active_deals,'recorded_market_matches',v_market_matches,'active_player_opportunities',v_active_opps,'deals',v_deal_items),
    'preparation',jsonb_build_object('active_authority_records',v_authority,'current_document_records',v_docs,'verification_status',v_p.verification_status),
    'open_player_requests',v_request_items,
    'service_priority_score',v_priority_score,
    'next_service_move',v_next_service,'next_control_fix',v_next_control,
    'truth_contract',jsonb_build_object(
      'service_score','Operational service-control score, not a measure of agent quality or player satisfaction.',
      'market_coverage','Recorded deals, matches and opportunities only; absence does not prove no external conversations exist.',
      'authority','No active authority record found means not recorded here, not that legal authority is absent.',
      'documents','Document presence is readiness context, not certification of legal completeness.',
      'player_contact_frequency','Not scored because the current interaction model is club/contact-centred rather than a reliable player communication ledger.'
    )
  );
end;
$$;

create or replace function public.platform_server_player_service_command(p_tenant_id uuid,p_limit integer default 50)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_total integer:=0;
  v_urgent integer:=0;
  v_service_risk integer:=0;
  v_attention integer:=0;
  v_free_agents integer:=0;
  v_contract_critical integer:=0;
  v_market_gaps integer:=0;
  v_active_deal_players integer:=0;
...
begin
  for v_item in
    ...
    order by case p.agency_priority when 'urgent' then 0 when 'high' then 1 when 'normal' then 2 else 3 end,p.next_action_due nulls first,p.updated_at desc
  loop
    v_card:=public.platform_server_player_service_card(p_tenant_id,v_item.id);
    v_total:=v_total+1;
    if (v_card->>'service_priority_score')::integer>=75 then v_urgent:=v_urgent+1; end if;
    if v_card->'service_control'->>'state'='service_risk' then v_service_risk:=v_service_risk+1;
    elsif v_card->'service_control'->>'state'='needs_attention' then v_attention:=v_attention+1; end if;
    if v_card->'career_timing'->>'market_trigger'='free_agent' then v_free_agents:=v_free_agents+1; end if;
    if v_card->'career_timing'->>'market_trigger'='contract_critical_window' then v_contract_critical:=v_contract_critical+1; end if;
    if v_card->'market_coverage'->>'state'='market_coverage_gap' then v_market_gaps:=v_market_gaps+1; end if;
    if coalesce((v_card->'market_coverage'->>'active_deals')::integer,0)>0 then v_active_deal_players:=v_active_deal_players+1; end if;
    if v_card->'player'->>'primary_staff_user_id' is null then v_unassigned:=v_unassigned+1; end if;
    if coalesce(v_card->'service_control'->'gaps','[]'::jsonb) ? 'player_next_action_overdue' then v_overdue_actions:=v_overdue_actions+1; end if;
    if coalesce((v_card->'service_control'->>'overdue_player_requests')::integer,0)>0 then v_overdue_requests:=v_overdue_requests+1; end if;
    if jsonb_array_length(v_cards)<v_limit then v_cards:=v_cards||jsonb_build_array(v_card); end if;
  end loop;

  select coalesce(jsonb_agg(x.value order by (x.value->>'service_priority_score')::integer desc,(x.value->'career_timing'->>'contract_days_remaining')::integer nulls last),'[]'::jsonb)
  into v_cards from jsonb_array_elements(v_cards) x;

  return jsonb_build_object(
    'tenant_id',p_tenant_id,'generated_at',now(),
    'summary',jsonb_build_object('active_players',v_total,'urgent_service_queue',v_urgent,'service_risk',v_service_risk,'needs_attention',v_attention,'free_agents',v_free_agents,'contract_critical_window',v_contract_critical,'market_coverage_gaps',v_market_gaps,'players_with_active_deals',v_active_deal_players,'players_without_primary_staff',v_unassigned,'overdue_player_next_actions',v_overdue_actions,'players_with_overdue_requests',v_overdue_requests,'visible_players',jsonb_array_length(v_cards),'hidden_by_limit',greatest(0,v_total-jsonb_array_length(v_cards))),
    'players',v_cards,
    'principle','Protect the player relationship by separating career urgency, market coverage and agency process control. Activity is not service quality unless it produces a current player plan.',
    'truth_contract',jsonb_build_object('priority','Deterministic attention score, not player value or transfer probability.','coverage','Recorded market activity only.','service','Operational control signals only, not player satisfaction.')
  );
end;
$$;

revoke all on function public.platform_server_player_service_card(uuid,uuid) from public,anon,authenticated;
revoke all on function public.platform_server_player_service_command(uuid,integer) from public,anon,authenticated;
grant execute on function public.platform_server_player_service_card(uuid,uuid) to service_role;
grant execute on function public.platform_server_player_service_command(uuid,integer) to service_role;;
