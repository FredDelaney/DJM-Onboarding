...
begin
  ...
  select jobid into v_jobid from cron.job where jobname = 'djm-official-football-refresh-daily' limit 1;
  if v_jobid is not null then perform cron.unschedule(v_jobid); end if;
  ...
  perform cron.schedule(
    ...
    $cmd$
      select net.http_post(
        ...
        body := jsonb_build_object('mode','refresh_all','source','official-football-daily'),
