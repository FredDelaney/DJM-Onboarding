create or replace function public.platform_server_deal_advantage_fast(p_tenant_id uuid,p_deal_room_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  v_direct_ctx:=public.platform_server_access_routes(p_tenant_id,v_deal.organisation_id,3);
  v_intro_ctx:=public.platform_server_introduction_routes(p_tenant_id,v_deal.organisation_id,3);
  ...
  begin v_direct:=coalesce((v_direct_ctx->'best_route'->>'route_score')::integer,0); exception when others then v_direct:=0; end;
  begin v_intro:=coalesce((v_intro_ctx->'best_route'->>'introduction_score')::integer,0); exception when others then v_intro:=0; end;
  ...
  if v_need.id is not null then v_demand:=case when v_need.need_type='confirmed' then least(100,78+coalesce(v_need.priority,3)*4) when v_need.need_type='predicted' then 55 else 50 end; else v_demand:=35; end if;
  ...
  if v_deal.club_need_id is not null then select count(*) into v_club_other_matches from djm_os.player_matches pm where pm.tenant_id=p_tenant_id and pm.club_need_id=v_deal.club_need_id and pm.player_id<>v_deal.player_id and pm.status in ('suggested','reviewing','shortlisted','pitched','active'); end if;
  ...
  return jsonb_build_object(
    ...
    'best_routes',jsonb_build_object('direct',v_direct_ctx->'best_route','introduction',v_intro_ctx->'best_route'),
    ...
    'truth_contract',jsonb_build_object('score','Deterministic operating-position score, not bargaining power, transfer probability or outcome forecast.','optionality','Recorded alternatives only, not a BATNA valuation.','competition','External competitor activity remains unknown unless explicitly recorded.')
  );
end;
$$;

create or replace function public.platform_server_negotiation_brief_fast(p_tenant_id uuid,p_deal_room_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
begin
  select * into v_deal from djm_os.deal_rooms d where d.id=p_deal_room_id and d.tenant_id=p_tenant_id;
  if not found then raise exception 'deal_not_found_for_tenant'; end if;
  if v_deal.player_id is null then return jsonb_build_object('available',false,'reason','negotiation_brief_currently_requires_player_deal'); end if;
  select * into v_player from public.players p where p.id=v_deal.player_id and p.tenant_id=p_tenant_id;
  ...
  v_direct:=public.platform_server_access_routes(p_tenant_id,v_deal.organisation_id,3);
  v_intro:=public.platform_server_introduction_routes(p_tenant_id,v_deal.organisation_id,3);
  v_momentum:=public.platform_server_deal_momentum(p_tenant_id,p_deal_room_id,30);
  ...
  v_questions:=v_questions||jsonb_build_array('What is the preferred outcome, acceptable fallback and issue that would cause us to stop or reframe the negotiation?','Which terms are confirmed facts, which are negotiating positions, and which are still assumptions?');
  ...
  v_unknowns:=v_unknowns||jsonb_build_array('competitor_activity_unless_explicitly_recorded','agent_concession_limits_unless_explicitly_recorded');

  return jsonb_build_object(
    'available',true,'tenant_id',p_tenant_id,'deal_room_id',p_deal_room_id,'generated_at',now(),'headline',v_deal.title,
    'known_recorded_facts',jsonb_build_object(
      'deal',jsonb_build_object('stage',v_deal.stage,'status',v_deal.status,'probability',coalesce(v_deal.probability,v_deal.manual_probability,v_deal.model_probability),'primary_blocker',v_deal.primary_blocker,'next_decision',v_deal.next_decision),
      'player',jsonb_build_object('name',trim(concat_ws(' ',v_player.first_name,v_player.last_name)),'current_club',v_player.current_club,'contract_status',v_player.contract_status,'contract_expiry',v_player.contract_expiry),
      'club',jsonb_build_object('name',v_org.name,'country',v_org.country,'league_name',v_org.league_name),
      'recorded_terms',jsonb_build_object('currency',v_deal.currency,'transfer_fee',v_deal.transfer_fee,'player_salary',v_deal.player_salary,'salary_period',v_deal.salary_period,'expected_commission',v_deal.expected_commission),
      'recorded_club_need',case when v_need.id is null then null else jsonb_build_object('need_type',v_need.need_type,'transfer_type',v_need.transfer_type,'transfer_budget',v_need.transfer_budget,'salary_budget',v_need.salary_budget,'currency',v_need.currency,'expires_at',v_need.expires_at) end
    ),
    ...
    'route_strategy',jsonb_build_object('direct_best_route',v_direct->'best_route','best_introduction_route',v_intro->'best_route'),
    'movement',jsonb_build_object('momentum_state',v_momentum->'state','momentum_score',v_momentum->'momentum_score','decision_pressure_state',v_pressure->'state','observed_review_due_at',v_pressure->'observed_review_due_at'),
    'unresolved_or_unrecorded',v_unknowns,'human_decisions_required',v_questions,
    'negotiation_guardrails',jsonb_build_object('minimum_acceptable_fee','not_recorded_unless_explicitly_set','player_salary_floor_or_target','not_recorded_unless_explicitly_set','commission_floor_or_protection','not_recorded_unless_explicitly_set','concession_order','not_recorded_unless_explicitly_set','walk_away_conditions','human_decision_required'),
    'truth_contract',jsonb_build_object('purpose','internal preparation brief only','known_facts','recorded database facts only','competition','unknown unless explicitly recorded','positions','floors, targets, concessions and walk-away terms are never invented')
  );
end;
$$;

create or replace function public.platform_server_deal_war_room_v4(p_tenant_id uuid,p_deal_room_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_advantage jsonb;
begin
  v_base:=public.platform_server_deal_war_room_v2(p_tenant_id,p_deal_room_id);
  v_readiness:=public.platform_server_negotiation_readiness(p_tenant_id,p_deal_room_id);
  v_sequence:=public.platform_server_negotiation_sequence(p_tenant_id,p_deal_room_id);
  v_advantage:=public.platform_server_deal_advantage_fast(p_tenant_id,p_deal_room_id);
  return v_base||jsonb_build_object(
    'negotiation',jsonb_build_object('score',v_readiness->'score','state',v_readiness->'state','gaps',v_readiness->'gaps','next_step',v_sequence->'next_step','blocking_gap_count',v_sequence->'blocking_gap_count'),
    'deal_advantage',jsonb_build_object('score',v_advantage->'operating_advantage_score','state',v_advantage->'state','edges',v_advantage->'edges','exposures',v_advantage->'exposures','unknowns',v_advantage->'unknowns'),
    'deep_detail',jsonb_build_object('negotiation_brief_available',true,'deal_advantage_detail_available',true,'load_strategy','on_demand')
  );
end;
$$;

create or replace function public.platform_server_deal_portfolio_v4(p_tenant_id uuid,p_limit integer default 50)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  for v_item in select x.value from jsonb_array_elements(coalesce(v_base->'deals','[]'::jsonb)) x loop
    ...
    v_advantage:=public.platform_server_deal_advantage_fast(p_tenant_id,(v_item->>'deal_room_id')::uuid);
    ...
    v_deals:=v_deals||jsonb_build_array(v_item||jsonb_build_object('negotiation_readiness_score',v_readiness->'score','negotiation_readiness_state',v_readiness->'state','deal_advantage_score',v_advantage->'operating_advantage_score','deal_advantage_state',v_advantage->'state','deal_advantage_edges',v_advantage->'edges','deal_advantage_exposures',v_advantage->'exposures'));
  end loop;
  return v_base||jsonb_build_object('deals',v_deals,'summary',(v_base->'summary')||jsonb_build_object('negotiation_ready_or_strong_count',v_ready,'negotiation_preparation_required_count',v_prepare,'strong_operating_position_count',v_strong,'exposed_operating_position_count',v_exposed),'principle','Attention, probability, control, momentum, negotiation preparation and deal advantage remain separate operating dimensions.');
end;
$$;

revoke all on function public.platform_server_deal_advantage_fast(uuid,uuid) from public,anon,authenticated;
revoke all on function public.platform_server_negotiation_brief_fast(uuid,uuid) from public,anon,authenticated;
revoke all on function public.platform_server_deal_war_room_v4(uuid,uuid) from public,anon,authenticated;
revoke all on function public.platform_server_deal_portfolio_v4(uuid,integer) from public,anon,authenticated;
grant execute on function public.platform_server_deal_advantage_fast(uuid,uuid) to service_role;
grant execute on function public.platform_server_negotiation_brief_fast(uuid,uuid) to service_role;
grant execute on function public.platform_server_deal_war_room_v4(uuid,uuid) to service_role;
grant execute on function public.platform_server_deal_portfolio_v4(uuid,integer) to service_role;
;
