...
begin
  ...
  begin v_profile_holds:=coalesce((v_readiness#>>'{summary,profile_or_verification_holds}')::int,0); exception when others then v_profile_holds:=0; end;
  ...
  v_next:=case
    ...
    when v_profile_holds>0 then jsonb_build_object('lane','external_profile_control','api_action','pitch_readiness','instruction','Resolve player/profile verification before external pitching.')
  ...
  return jsonb_build_object(
    ...
    'executive_summary',jsonb_build_object(
      ...
      'external_profile_holds',v_profile_holds,
    ...
    'truth_contract',jsonb_build_object(
      'layers','Career permission, external profile readiness, pitch delivery, browser telemetry, explicit response and deal state are separate facts.',
      ...
      'automation','No external message, deal-stage change or market-strategy change occurs automatically from this command.',
