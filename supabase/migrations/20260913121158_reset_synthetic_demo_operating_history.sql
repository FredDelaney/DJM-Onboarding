...
begin
  ...
  where tenant_id=p_tenant_id and (
    action like 'agency_action.%' or
    metadata->>'source'='agency_command_engine' or
    entity_type='agency_action'
  );
  ...
  return jsonb_build_object('tenant_id',p_tenant_id,'reset',true,'deleted',jsonb_build_object(
    ...
    'proposals',v_proposals,'pulse_events',v_pulse,'audit_events',v_audit));
end;
$$;
...
revoke all on function public.platform_server_reset_demo_operating_history(uuid) from public,anon,authenticated;
