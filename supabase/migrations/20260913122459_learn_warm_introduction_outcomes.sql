alter function platform.evaluate_agency_action_outcome(uuid) rename to evaluate_agency_action_outcome_core_v2;
...
declare
  ...
  v_key text:='introduction_outcome_pending';
...
begin
  ...
  if v_p.action_type<>'create_introduction_task' then
    return platform.evaluate_agency_action_outcome_core_v2(p_proposal_id);
  end if;

  v_window_end:=coalesce(v_p.applied_at,v_p.created_at)+interval '14 days';
  begin v_target_person:=nullif(v_p.proposed_payload->>'target_person_id','')::uuid; exception when others then v_target_person:=null; end;
  begin v_org:=nullif(v_p.proposed_payload->>'organisation_id','')::uuid; exception when others then v_org:=null; end;
  begin v_intermediary:=nullif(v_p.proposed_payload->>'intermediary_person_id','')::uuid; exception when others then v_intermediary:=null; end;
  ...
  if v_p.status='undone' then
    ...
    select count(*),min(i.occurred_at) into v_interactions,v_first_observed
    from djm_os.interactions i
    where i.tenant_id=v_p.tenant_id
      and i.occurred_at>=coalesce(v_p.applied_at,v_p.created_at)
      and ((v_target_person is not null and i.person_id=v_target_person) or (v_org is not null and i.organisation_id=v_org));

    if v_interactions>0 then
      v_downstream:='positive'; v_key:='target_club_interaction_after_introduction_task';
    elsif now()>=v_window_end then
      v_downstream:='neutral'; v_key:='no_target_club_interaction_in_window';
    else
      v_downstream:='pending'; v_key:='introduction_outcome_pending';
    end if;
  end if;

  v_evidence:=jsonb_build_object(
    'intermediary_person_id',v_intermediary,'target_person_id',v_target_person,'organisation_id',v_org,
    'target_interactions_after_action',v_interactions,'commitment_status',v_commitment.status,'task_id',v_commitment.task_id,
    'success_definition','A fresh interaction with the target contact or target organisation. Contacting the intermediary alone does not count as downstream success.'
  );
...
as $$
declare
  v_p record; v_result jsonb; v_results jsonb:='[]'::jsonb; v_checked integer:=0; v_failed integer:=0;
begin
  ...
  for v_p in select p.id from platform.agency_action_proposals p
    where p.tenant_id=p_tenant_id and p.created_at>=now()-interval '180 days' and p.status in ('applied','undone','failed') order by p.created_at desc
  loop
    begin
      v_result:=platform.evaluate_agency_action_outcome(v_p.id); v_results:=v_results||jsonb_build_array(v_result); v_checked:=v_checked+1;
    exception when others then
      v_failed:=v_failed+1; v_results:=v_results||jsonb_build_array(jsonb_build_object('proposal_id',v_p.id,'error',left(sqlerrm,500)));
    end;
  end loop;
...
revoke all on function platform.evaluate_agency_action_outcome_core_v2(uuid) from public,anon,authenticated;
...
grant execute on function platform.evaluate_agency_action_outcome_core_v2(uuid) to service_role;
