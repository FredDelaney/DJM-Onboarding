...
begin
  for v_t in
    select t.id tenant_id,t.slug,t.display_name,coalesce(l.stage,'onboarding') stage,l.onboarding_status,l.trial_started_at,l.trial_ends_at,l.contracted_monthly_cents,l.contract_currency,l.account_owner_name
    ...
    order by case coalesce(l.stage,'onboarding') when 'at_risk' then 1 when 'trial' then 2 when 'onboarding' then 3 when 'demo' then 4 when 'live' then 5 else 6 end,t.display_name
