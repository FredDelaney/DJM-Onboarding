...
declare
  ...
  v_task_actions text[]:=array[
    ...
    'create_deal_blocker_task','create_deal_decision_task'
  ];
begin
    ...
    if found then
      ...
      values(new.tenant_id,new.id,new.command_id,v_task.id,coalesce(new.approved_by,new.requested_by),v_status,v_task.due_at,v_task.completed_at,
        jsonb_build_object(
          ...
          'deal_room_id',new.proposed_payload->>'deal_room_id','deal_step_type',new.proposed_payload->>'deal_step_type'
        ))
...
revoke execute on function platform.sync_commitment_from_proposal() from public,anon,authenticated;
