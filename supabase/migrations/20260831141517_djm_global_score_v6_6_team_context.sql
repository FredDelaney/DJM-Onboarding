...
declare
 ...
 v_w_comp numeric:=0; v_w_team numeric:=0; v_w_role numeric:=0; v_w_prod numeric:=0; v_w_market numeric:=0; v_w_career numeric:=0; v_observed_weight numeric:=0; v_prior_strength numeric:=45;
...
begin
 ...
 teamctx:=djm_os.subject_team_context(p_subject_id); v_team:=djm_os.safe_json_number(teamctx->>'score'); v_team_q:=coalesce(djm_os.safe_json_number(teamctx->>'quality'),0);
 ...
 -- V6.6 nominal score weights sum to 100 and every missing component remains unknown.
 ...
 v_conf:=round(100*least(1,.25*least(1,v_observed_weight/100.0)+.12*v_country_q+.08*v_team_q+.18*v_identity_q+.13*v_role_q+.06*v_peer_q+.04*v_market_q+.07*v_career_q+.07*v_source_diversity));
