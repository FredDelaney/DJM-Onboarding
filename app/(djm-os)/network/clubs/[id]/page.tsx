  ...
  return (
    <DjmOsShell eyebrow="Club relationship workspace" title={org?.name||'Club'}>
      ...
      {!data ? <div className="djm-os-empty"><Building2 size={25}/><p>{busy?'Loading club…':'Club not found.'}</p></div> : <>
        ...
        <div className="djm-os-grid djm-os-grid-2">
          <section className="djm-os-panel"><div className="djm-os-panel-head"><div><h2>Club needs</h2><p>Requests flow into Market and matching automatically.</p></div><Target size={19}/></div>{(data.needs||[]).length?<div className="djm-os-list">{data.needs.map((need:any)=><Link href="/market" className="djm-os-list-row" style={{textDecoration:'none',color:'inherit'}} key={need.id}><div style={{flex:1}}><strong>{need.title||need.position}</strong><p>{[need.position,need.preferred_foot,need.transfer_type].filter(Boolean).join(' · ')}</p><small>{need.status} · {need.match_count||0} matches</small></div><div className="djm-os-score"><b>{Math.round(Number(need.top_match_score||0))}</b><small>best match</small></div></Link>)}</div>:<Empty text="No club needs recorded."/>}</section>
