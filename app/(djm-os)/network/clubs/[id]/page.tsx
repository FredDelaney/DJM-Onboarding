...
import { compactDateTime, platformRpc, friendlyError, initials } from '@/lib/platform-client';
...
export default function ClubWorkspacePage() {
  ...
  const load = async () => {
    ...
    try {
      const result:any = await platformRpc('djm_network_club_workspace', { p_organisation_id: id });
      setData(result);
  ...
  const saveClub = async (event:FormEvent) => {
    ...
    setBusy(true); setError('');
    try {
      await platformRpc('djm_network_update_club_profile', {
  ...
  const closeTask = async (taskId:string) => {
    try { await platformRpc('djm_network_set_task_status',{p_task_id:taskId,p_status:'completed'}); await load(); }
    catch(e){ setError(friendlyError(e)); }
  };

  const deleteClub = async () => {
    try {
      const impact:any=await platformRpc('djm_delete_preview',{p_entity_type:'club',p_entity_id:id});
      ...
      await platformRpc('djm_delete_entity',{p_entity_type:'club',p_entity_id:id,p_confirm:true});
      router.push('/network');
    } catch(e){ setError(friendlyError(e)); }
  };
  ...
  return (
    <AgencyShell eyebrow="Club relationship workspace" title={org?.name||'Club'}>
