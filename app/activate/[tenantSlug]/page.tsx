...
export default function AgencyLaunchPage() {
  ...
  const runtime = {
    resolved: Boolean(tenantSlug),
    slug: tenantSlug,
    branding: {
      display_name: 'Agency workspace',
      portal_name: 'Owner workspace',
  ...
  const loadLaunch = useCallback(async () => {
    ...
    try {
      ...
      setLaunch(next);
      if (next?.branding?.display_name) {
        document.title = `${next.branding.display_name} | Owner launch`;
      }
      if (next?.branding) {
        setBrand({
